...
module.exports = mock => {
  ...
  const useSchema = (schema, obj, uid, options) => {
    ...
    const genUid = uid => {
      ...
      if (uid.length === 6) {
        ...
        return genUid(`${uid}${'0'.repeat(6 - uid.length)}`);
      }
    };
    ...
    return {
      ...
      [key]: {
        ...
        [schemaName]: function(obj) {
          return useSchema(schema, obj, this.collection.length, options);
        }, // Get context (db) to return an object with a generated uid
        ...appendCrud(schemaName), // Auto generate crud
        ...resolvers // resolvers
      }
    };
  }, {});
